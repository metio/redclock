...
pub trait RedmineClient {
    ...
    fn track_project_time(&self, activity_id: u64, project_id: u64, option: Option<&String>, elapsed_hours: f64) -> anyhow::Result<()>;
    fn track_issue_time(&self, activity_id: u64, issue_id: u64, option: Option<&String>, elapsed_hours: f64) -> anyhow::Result<()>;
}
...
impl RedmineHttpClient {
    pub const fn new(redmine: Redmine, cache_directory: PathBuf, activities_fetch_interval_seconds: u64, projects_fetch_interval_seconds: u64, issues_fetch_interval_seconds: u64) -> Self {
        Self { redmine, cache_directory, activities_fetch_interval_seconds, projects_fetch_interval_seconds, issues_fetch_interval_seconds }
    }

    fn should_read_from_cache(cache_timestamp_path: &PathBuf, fetch_interval_seconds: u64) -> anyhow::Result<bool> {
...
impl RedmineClient for RedmineHttpClient {
    fn get_all_activities(&self) -> anyhow::Result<Vec<TimeEntryActivity>> {
        ...
        if cache_path.exists() && cache_timestamp_path.exists() && Self::should_read_from_cache(&cache_timestamp_path, self.activities_fetch_interval_seconds)? {
            // read from cache
            ...
            fs::write(&cache_timestamp_path, format!("{}", Self::current_time_in_seconds()?))?;
    ...
    fn get_all_projects(&self) -> anyhow::Result<Vec<Project>> {
        ...
        if cache_path.exists() && cache_timestamp_path.exists() && Self::should_read_from_cache(&cache_timestamp_path, self.projects_fetch_interval_seconds)? {
            // read from cache
            let cache_content = fs::read_to_string(&cache_path)?;
            Ok(serde_json::from_str(&cache_content)?)
        } else {
    ...
    fn get_all_open_issues(&self) -> anyhow::Result<Vec<Issue>> {
        ...
        if cache_path.exists() && cache_timestamp_path.exists() && Self::should_read_from_cache(&cache_timestamp_path, self.issues_fetch_interval_seconds)? {
            // read from cache
            let cache_content = fs::read_to_string(&cache_path)?;
            Ok(serde_json::from_str(&cache_content)?)
        } else {
            // fetch from server
            let endpoint = ListIssues::builder().status_id(IssueStatusFilter::Open).build()?;
            let issues= self
                .redmine
    ...
    fn track_project_time(&self, activity_id: u64, project_id: u64, comment: Option<&String>, elapsed_hours: f64) -> anyhow::Result<()> {
        let endpoint = CreateTimeEntry::builder()
            ...
            .build()?;
        self.redmine.json_response_body::<_, TimeEntryWrapper<TimeEntry>>(&endpoint)?;
        Ok(())
    }

    fn track_issue_time(&self, activity_id: u64, issue_id: u64, comment: Option<&String>, elapsed_hours: f64) -> anyhow::Result<()> {
        let endpoint = CreateTimeEntry::builder()
            .activity_id(activity_id)
            .issue_id(issue_id)
            .hours(elapsed_hours)
